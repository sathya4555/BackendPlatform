...
import { approleDTO } from '../../Dtos/approleDTO';
...
@Controller('approle-data')
export class ApproleRoutes{

  constructor(private app_messagesFacade : AppRoleFacade) { }
    ...
    for (var i = 0; i < this.topicArray.length; i++) {
      this.sns_sqs.listenToService(this.topicArray[i], this.serviceName[i], (() => {
        ...
        return async (result) => {
          ...
          try {
            let responseModelOfApp_MessagesDto: ResponseModel<approleDTO> = null;
            ...
            let requestModelOfApp_MessagesDto: RequestModel<approleDTO> = result["message"];
  ...
  @Post("") 
  async createApp_Messages(@Body() body:RequestModel<approleDTO>): Promise<ResponseModel<approleDTO>> {  //requiestmodel<STUDENTDto></STUDENTDto>....Promise<ResponseModel<Grou[pDto>>]
    try {
  ...
  @Put("/")
  async updateapp_messages(@Body() body: RequestModel<approleDTO>): Promise<ResponseModel<approleDTO>> {  //requiestmodel<STUDENTDto></STUDENTDto>....Promise<ResponseModel<Grou[pDto>>]
    try {
  ...
  @Delete('/')
  deleteApp_Messages(@Body() body:RequestModel<approleDTO>): Promise<ResponseModel<approleDTO>>{
    try {
