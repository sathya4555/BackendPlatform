...
// import { clienDTO } from '../../Dtos/clienDTO';
...
export class ClientRoutes{
  ...
  constructor(private app_messagesFacade : ClientFacade) { }
    ...
    for (var i = 0; i < this.topicArray.length; i++) {
      this.sns_sqs.listenToService(this.topicArray[i], this.serviceName[i], (() => {
        ...
        return async (result) => {
          ...
          try {
            let responseModelOfApp_MessagesDto: ResponseModel<clienDTO> = null;
            ...
            let requestModelOfApp_MessagesDto: RequestModel<clienDTO> = result["message"];
  ...
  @Post("") 
  async createApp_Messages(@Body() body:RequestModel<clienDTO>): Promise<ResponseModel<clienDTO>> {  //requiestmodel<STUDENTDto></STUDENTDto>....Promise<ResponseModel<Grou[pDto>>]
    try {
  ...
  @Put("/")
  async updateapp_messages(@Body() body: RequestModel<clienDTO>): Promise<ResponseModel<clienDTO>> {  //requiestmodel<STUDENTDto></STUDENTDto>....Promise<ResponseModel<Grou[pDto>>]
    try {
  ...
  @Delete('/')
  deleteApp_Messages(@Body() body:RequestModel<clienDTO>): Promise<ResponseModel<clienDTO>>{
    try {
