...
export class RoleRoutes{
  ...
  constructor(private app_messagesFacade : RoleFacade) { }
    ...
    for (var i = 0; i < this.topicArray.length; i++) {
      this.sns_sqs.listenToService(this.topicArray[i], this.serviceName[i], (() => {
        ...
        return async (result) => {
          ...
          try {
            let responseModelOfApp_MessagesDto: ResponseModel<roleDTO> = null;
            ...
            let requestModelOfApp_MessagesDto: RequestModel<roleDTO> = result["message"];
  ...
  @Post("") 
  async createApp_Messages(@Body() body:RequestModel<roleDTO>): Promise<ResponseModel<roleDTO>> {  //requiestmodel<STUDENTDto></STUDENTDto>....Promise<ResponseModel<Grou[pDto>>]
    try {
  ...
  @Put("/")
  async updateapp_messages(@Body() body: RequestModel<roleDTO>): Promise<ResponseModel<roleDTO>> {  //requiestmodel<STUDENTDto></STUDENTDto>....Promise<ResponseModel<Grou[pDto>>]
    try {
  ...
  @Delete('/')
  deleteApp_Messages(@Body() body:RequestModel<roleDTO>): Promise<ResponseModel<roleDTO>>{
    try {
