...
@Controller('feature-data')
export class FeatureRoutes{

  constructor(private app_messagesFacade : FeatureFacade) { }
    ...
    for (var i = 0; i < this.topicArray.length; i++) {
      this.sns_sqs.listenToService(this.topicArray[i], this.serviceName[i], (() => {
        ...
        return async (result) => {
          ...
          try {
            let responseModelOfApp_MessagesDto: ResponseModel<featureDTO> = null;
            ...
            let requestModelOfApp_MessagesDto: RequestModel<featureDTO> = result["message"];
            ...
            for (let index = 0; index < result.OnFailureTopicsToPush.length; index++) {
              ...
              let errorResult: ResponseModel<featureDTO> = new ResponseModel<featureDTO>(null,null,null,null,null,null,null,null,null);;
  ...
  @Post("") 
  async createApp_Messages(@Body() body:RequestModel<featureDTO>): Promise<ResponseModel<featureDTO>> {  //requiestmodel<STUDENTDto></STUDENTDto>....Promise<ResponseModel<Grou[pDto>>]
    try {
  ...
  @Put("/")
  async updateapp_messages(@Body() body: RequestModel<featureDTO>): Promise<ResponseModel<featureDTO>> {  //requiestmodel<STUDENTDto></STUDENTDto>....Promise<ResponseModel<Grou[pDto>>]
    try {
  ...
  @Delete('/')
  deleteApp_Messages(@Body() body:RequestModel<featureDTO>): Promise<ResponseModel<featureDTO>>{
    try {
